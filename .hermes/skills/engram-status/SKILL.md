---
name: engram-status
description: Momentaufnahme des Lernstands — Text sofort, optional als lokale HTML-Seite. Welche Quellen eingebunden sind, wo jeder Lernpfad steht, was heute und in den kommenden Wochen fällig wird. Use when the user asks "wo stehe ich", "was ist fällig", "gib mir eine Übersicht", or wants a status page, and when a session-start upstream-sync notice needs no separate command.
version: 1.0.0
argument-hint: [text|kurz | seite]
---

# Engram — Standortbestimmung (Hermes)

Momentaufnahme: Quellen, Lernpfad-Stand, kommende Fälligkeiten.

> **Einzige bewusste Abweichung in der `description` gegenüber der Claude-Fassung:**
> dort steht „geteilte Seite" / „shareable status page". Hermes hat kein
> Artifact-Publishing — die Seite wäre eine Datei auf der Platte. Eine `description`,
> die etwas verspricht, was die Plattform nicht kann, ist ein Defekt im Skill-Index und
> kein Formalverstoß. Nicht „korrigieren"; siehe `.hermes/PLATTFORM.md` §5.
...
2. **`$ENGRAM_ROOT/.claude/skills/engram-status/SKILL.md` vollständig lesen** und wörtlich
