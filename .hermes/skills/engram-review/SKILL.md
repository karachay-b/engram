...
# Engram — die Zwei-Minuten-Gewohnheit (Hermes)

Fällige Wiederholungen abarbeiten, freies Abrufen, FSRS-Bewertung.
...
2. **`$ENGRAM_ROOT/.claude/skills/engram-review/SKILL.md` vollständig lesen** und wörtlich
   befolgen — mit den Übersetzungen aus Schritt 1. Dort stehen die Regeln dieses
   Forks; sie sind nicht zusammenfassbar.
3. Jene Datei verlangt ihrerseits **`$ENGRAM_ROOT/skills/review/SKILL.md` in voller
   Länge** — der unveränderte Upstream-Skill, der die eigentliche Arbeit
   beschreibt. Auch den wörtlich befolgen, nicht paraphrasieren.
   Seinen Engine-Resolver kannst du überspringen: `$ENGRAM_ROOT` steht schon.
