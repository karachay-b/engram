...
<p align="center">
  <img src="https://img.shields.io/badge/version-1.15.0-6D4AA8.svg" alt="Version 1.15.0">
...
<sub>ZCode: Claude Code-compatible plugin surfaces, so the port is manifest + one hook wrapper + docs. The nudge needs a wrapper because ZCode discards plain SessionStart stdout (JSON `additionalContext` only) — the shared hooks file carries both entries and the wrapper self-suppresses where plain stdout already delivers. The blind assessor runs as a fresh-context generic Agent child; declared plugin agents are diagnostic-only in ZCode 3.9.2. A live model-driven session is not yet recorded — first-run reports welcome. Details in [INSTALL-ZCODE.md](INSTALL-ZCODE.md).</sub><br>
